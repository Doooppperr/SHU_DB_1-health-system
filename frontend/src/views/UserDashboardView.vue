<template>
  <div class="workspace-page">
    <section class="welcome-panel"><div><p>统一健康视图</p><h2>今天也要照顾好自己</h2><span>机构体检与日常自测已汇入同一条时间线。</span></div><div class="welcome-actions"><el-button type="primary" @click="router.push({name:'measurements'})">记录日常测量</el-button><el-button type="primary" @click="router.push({name:'registrations'})">登记体检</el-button></div></section>
...
const metrics=computed(()=>{const counts={self_measurement:0,exam_registration:0,institution_report:0};events.value.forEach(e=>{if(counts[e.type]!==undefined)counts[e.type]++});return[{label:"时间线事件",value:events.value.length,icon:"线",note:"统一健康记录"},{label:"日常测量",value:counts.self_measurement,icon:"测",note:"保留全部原始值"},{label:"机构报告",value:counts.institution_report,icon:"报",note:"标准化只读数据"},{label:"体检登记",value:counts.exam_registration,icon:"登",note:"等待机构提交"}]});
