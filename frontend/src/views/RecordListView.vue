...
import { createRecord, deleteRecord, fetchRecords } from "../api/records";
...
const openCreateDialog = async () => {
  ...
  if (manageableFriends.value.length === 0) {
    try {
      await loadFriends();
    } catch (error) {
...
onMounted(async () => {
  ...
  await Promise.all([loadRecords(), loadInstitutions(), loadFriends()]);
