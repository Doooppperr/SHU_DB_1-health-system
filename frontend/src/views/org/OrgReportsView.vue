<template>
  <div class="workspace-page report-workspace">
    ...
    <section v-if="view!=='history'" class="report-toolbar">
    ...
    <section v-if="view!=='history'" class="appointment-card-list" v-loading="loading">
    ...
    <section v-else class="archive-grid" v-loading="loading">
      <article v-for="item in archivedReports" :key="item.id" class="archive-card">
        <header><span>已交付</span><strong>{{item.exam_date}}</strong></header>
        <h3>{{item.subject_name_snapshot || '受检者'}}</h3>
        <p>{{item.institution_name || '本机构'}} · {{item.appointment_id?'预约体检':'历史健康数据'}}</p>
      ...
      <el-empty v-if="!loading&&!archivedReports.length" description="暂无已归档健康数据" />
    </section>
    ...
    <el-drawer v-model="detailVisible" title="健康数据生产" size="min(960px,98vw)" class="report-detail-drawer">
      <template v-if="current">
        ...
        <section class="production-section">
          ...
          <article v-for="item in current.assets||[]" :key="item.id" class="asset-result"><span><strong>{{item.title}}</strong><small>{{item.annotation||'暂无机构批注'}}</small></span><el-button v-if="current.status==='draft'" link type="danger" @click="removeAsset(item)">删除</el-button></article>
        </section>
...
async function load(){loading.value=true;try{const [appointmentResponse,reportResponse]=await Promise.all([fetchOrgAppointments(status.value?{status:status.value}:{}),fetchOrgReports({status:"published"})]);items.value=appointmentResponse.data.items||[];archivedReports.value=reportResponse.data.items||[];}catch(error){ElMessage.error(error?.response?.data?.message||"体检任务加载失败");}finally{loading.value=false;}}
