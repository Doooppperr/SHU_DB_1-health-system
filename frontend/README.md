...
普通用户只能预约、取消未履约预约和只读查看机构体检报告，不能创建、OCR 上传、补录、确认、修改或删除报告。
...
- 机构运营总览；
- 预约体检管理、每日名额设置与体检报告生产；
- 机构资料维护；
- 机构相册管理；
- 套餐变更申请和信息审核记录。

`/org/reports` 已命名为“体检管理”。机构先把未履约预约确认到检为待上传报告，再手工或 OCR 建立草稿；受检者、日期和套餐由预约锁定，只复核指标。提交后报告永久归档、预约变已履约，不提供撤下操作；历史未绑定预约报告继续只读展示。
...
当前 15 个测试文件、88 项测试覆盖认证/角色映射、多标签页会话隔离、预约/审核角色路由、机构封面、亲友健康对象筛选、AI 按需加载、RAG 检索状态与流式交互、关怀模式/布局、账号注册载荷、图表外观和 HTTP 行为。生产构建与依赖审计结果见 [`../项目文档/测试报告.md`](../项目文档/测试报告.md)。
