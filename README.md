...
康康健健 HealthDoc 是一个基于 Flask 与 Vue 3 的 B/S 健康管理项目。本地开发默认使用 SQLite schema v3。普通用户维护个人健康资料、体检登记和日常自测，体检机构生产标准化报告，系统通过精确匹配把报告发布给用户。项目同时提供公开门户、机构服务、亲友授权、评论、OCR、健康 AI、关怀模式和三角色工作台。

## 当前实现

- 公开门户：`/` 展示项目介绍、核心能力、使用流程、隐私提示和关于我们；登录、注册使用独立页面并保留图片验证码。
- 普通用户：进入 `/dashboard`，维护个人资料与不可修改的健康身份码；可从总览快捷按钮或侧栏直接进入 `/registrations` 登记体检，并录入六类日常测量、查看统一健康时间线、指标趋势和机构发布的只读报告。
- 机构账号：进入 `/org/dashboard`，维护所属机构资料、相册与套餐，通过手工或 OCR 创建报告，复核指标后锁定、提交匹配或撤下。一个机构可绑定多个同权限账号。
...
│  │  ├─ health/、exam_reports/        # 自测、登记、时间线、趋势、用户报告
│  │  ├─ org/、admin/                  # 机构与系统管理工作区
│  │  ├─ friends/、comments/、ai/      # 延续的亲友、评论与 AI
│  │  └─ services/                     # OCR、匹配、存储与权限服务
...
新空库首次启动会直接创建 v3。已有非 v3 数据库不会被 `db.create_all()` 半升级；应用会提示执行升级脚本：
...
3. 在同目录临时文件创建完整 v3 结构；
4. 验证表、列、约束、外键和 `integrity_check`；
5. 生成 `health_system.before-schema-v3-时间戳-随机串.db` 备份；
6. 原子替换正式数据库。

当前本地库已完成迁移，`PRAGMA user_version=3`、`integrity_check=ok`、外键违规为 0。升级前备份仍保留，不能在确认不需要前删除。
...
| 角色 | 登录后入口 | 当前权限 |
|---|---|---|
| 普通用户 `user` | `/dashboard` | 个人资料、体检登记、自测、时间线、趋势、报告、亲友、机构、评论和健康 AI |
...
演示快照包含 13 个账号、138 条日常测量、20 条体检登记、12 份机构报告和 72 条标准化报告指标。5 个普通用户都拥有丰富时间线，覆盖已发布、等待匹配、已撤下、同日多次自测、机构指标优先、部分指标回退和亲友授权。完整账号矩阵见[测试账号与演示数据](项目文档/测试账号与演示数据.md)。
...
- 用户登记体检时选择机构、日期及可选套餐；同一用户同一体检日期只能有一个未取消登记。
- 报告精确匹配条件为真实姓名、健康身份码、体检日期和当前机构，任何一项不同都不返回候选用户。
- 机构可先提交、用户后登记，也可用户先登记、机构后提交。未匹配报告最多保留 60 天，过期后禁止匹配并由启动清理、惰性清理或独立命令删除。
- 报告状态为 `draft → locked → waiting_match/published → withdrawn`。离开草稿后内容不可修改；撤下后报告正文不可读取，时间线仅保留“机构已撤下”状态事件，趋势和 AI 立即排除报告指标。
...
- `backend/instance/health_system.db`
- `backend/instance/health_system.before-schema-v3-*.db`
...
- [数据库设计说明](项目文档/数据库设计说明.md)与[数据库规范化说明](项目文档/数据库规范化说明.md)：schema v3 物理模型、迁移、约束和范式。
