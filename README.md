...
康康健健 HealthDoc 是一个基于 Flask 与 Vue 3 的 B/S 健康服务平台，本地开发默认使用 SQLite schema v8。普通用户通过健康总览、健康时间线、体检数据和趋势管理跨来源资料，可为本人或已授权亲友一次安排最多五人的同行体检；机构分院围绕接待、到检、报告整理和提交开展工作，同一机构主体的兄弟分院可只读查看彼此已归档报告。平台同时提供套餐版本审核、空位提醒、报告识别导入、私有检查附件、跨院访问审计、健康 AI、关怀模式和三角色工作台。
...
| 3.0 | 平台化 UI、健康领域、套餐版本、多人预约、候补通知、图文报告和分院协作 | 当前运行版本；数据库基线为 schema v8 |

版本演进不是三套并存系统。当前代码、测试、演示数据和文档共同描述一套融合后的 3.0 平台，旧版本能力只在仍然有效时继续保留。

## 当前实现（3.0/schema v8）

- 公开门户：`/` 展示项目介绍、核心能力、使用流程、隐私提示和关于我们；登录、注册使用独立页面并保留图片验证码。注册必须填写通知邮箱，但邮箱不是登录账号键，家庭成员或演示账号可以共用同一收件箱。
- 普通用户：进入 `/dashboard`，通过“记录今日测量”抽屉录入六类日常数据；健康时间线同时呈现一次体检一张旅程卡和按自然日聚合的本人记录。健康趋势直接标注每个数据点、适用的参考范围与来源，并在本次页面授权后自动生成 AI 图表解读。体检预约使用分步引导选择日期、机构、套餐和受检者。
...
| 数据库 | 本地 SQLite schema v8；GaussDB/openGauss 使用 Alembic v8 迁移 |
...
│  ├─ uploads/                         # 30 个 demo-v8 水印素材受控跟踪，其余运行时文件忽略
...
新空库首次启动会直接创建 v8。已有非 v8 数据库不会被 `db.create_all()` 半升级；应用会提示执行升级脚本。v7→v8 在保留原分院主键与业务外键的前提下增加机构主体、分院归属和跨院访问审计；旧 v4–v7 SQLite 也可由同一脚本全量保留升级：
...
3. 在同目录临时文件创建完整 v8 结构；
4. 验证表、列、约束、外键和 `integrity_check`；
5. 生成带时间戳的升级前完整备份；
6. 原子替换正式数据库。

当前本地库为 `PRAGMA user_version=8`，`integrity_check=ok`、外键违规为 0。升级前备份仍需保留到人工验收结束。

重建本地演示数据时，先只读检查，再显式确认覆盖业务演示记录。脚本逐字段保留原 13 个账号及密码哈希，并补齐 12 个新增分院账号：
...
演示快照包含 25 个账号、5 个机构主体、15 家分院、25 个套餐、40 个预约组、56 位预约参与者、50 份已归档体检报告、120 条日常测量、15 个私有报告附件和 15 张分院封面。五个主体的分院数为 5/4/3/2/1，套餐为 8/6/5/4/2，报告与预约也按不同经营规模分布，不再使用整齐复制的数据矩阵。5 个普通用户分别承担不同现实生活场景，机构账号可验证本院生产、兄弟分院归档只读、单院空状态和跨机构拒绝。完整账号矩阵见[测试账号与演示数据](项目文档/测试账号与演示数据.md)。
...
当前验收基线：后端 56 项、前端 103 项测试通过，Vite production build 通过；SQLite/openGauss v8 结构、v7→v8 升级、演示数据预检与覆盖流程已验证。
...
- [数据库设计说明](项目文档/数据库设计说明.md)与[数据库规范化说明](项目文档/数据库规范化说明.md)：schema v8 机构主体/分院物理模型、迁移、约束和范式。
...
- [服务器部署与同步](项目文档/服务器部署与同步.md)：生产拓扑、schema v8 数据同步、备份、回滚和发布验收。
