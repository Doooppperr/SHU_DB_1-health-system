...
def apply_seed():
    ...
    missing_codes = set(BASE) - set(definitions)
    if missing_codes:
        raise RuntimeError(
            "the demo generator is missing indicator codes: "
    ...
    try:
        ...
        for profile, user in enumerate(users):
            for index in range(RECORDS_PER_USER):
                ...
                package = packages_by_institution.get(institution.id)
                staff = (
                ...
                if staff is None:
                    raise RuntimeError(
                        f"institution {institution.id} has no seeded staff account"
                    )
                examined_at = datetime.combine(
                    _exam_date(index), datetime.min.time(), tzinfo=timezone.utc
                )
                record = HealthRecord(
                    ...
                    status="published",
                    locked_at=examined_at,
                    submitted_at=examined_at,
                    published_at=examined_at,
                )
