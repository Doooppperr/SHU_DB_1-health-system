...
Flask 后端负责公开认证、三角色授权、个人健康资料、日常测量、体检预约与履约、机构报告永久归档、套餐变更审核、亲友只读授权、评论、OCR 和健康 AI。本地使用 SQLite schema v6；服务器通过 `DATABASE_URL` 连接 GaussDB/openGauss，并支持从已验证 SQLite 快照执行显式全量同步。
...
默认数据库为 `instance/health_system.db`。仓库跟踪这一份只含合成数据的演示快照，包含 5 个普通用户的丰富时间线和预约状态；账号见 [`../项目文档/测试账号与演示数据.md`](../项目文档/测试账号与演示数据.md)。SQLite 连接启用外键；`PRAGMA user_version=6` 标识当前结构。新空库会直接创建 v6，v5 使用升级脚本全量保留迁移。
...
v4 升级到 v5 时会保留全部业务数据并原子替换数据库；更早的不支持结构只保留当前系统管理员账号及密码哈希。原数据库始终完整保存为时间戳备份。当前核心表包括：
...
| `/api/appointments` | 普通用户 | 未来 30 天余量、预约列表、创建与取消 |
...
预约状态为 `unfulfilled → awaiting_report → fulfilled`；`unfulfilled` 可由用户取消为 `cancelled`，或由机构随时置为不可恢复的 `invalidated`。失效事件按预约日期进入本人及获授权亲友健康时间线。每日上限为空表示不限量，正整数表示限制；降低上限不取消既有预约。
...
可分析对象为本人或已授权亲友的 `published` 机构报告。精确 `selected_record_ids` 与 `record_scope: {"owner_id": 2, "mode": "all_confirmed"}` 互斥，后者在 schema v6 中解析为该归属人的全部已发布报告；两种方式都必须逐请求同意并重新鉴权。
...
当前 31 项测试使用独立内存 SQLite，不修改 `instance/health_system.db`；覆盖 schema v6、v5 全量保留升级、预约容量与状态机、套餐审批/驳回/撤回、机构报告永久归档、亲友只读边界、趋势/隐私、AI 同意、RAG、OCR 文件删除及全量快照迁移。完整结论见 [`../项目文档/测试报告.md`](../项目文档/测试报告.md)。
