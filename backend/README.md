...
当前 51 项测试使用独立内存 SQLite，不修改 `instance/health_system.db`；覆盖 schema v7、旧库全量保留升级、演示数据安全重建、健康领域、套餐版本、多人预约组、候补提醒、Outbox 常驻领取且不重复发送、连续中文邮件正文、报告指标/文字/附件、亲友只读边界、趋势与时间线、AI 同意、RAG 已核验缓存降级、识别导入临时文件删除及全量快照迁移。完整结论见 [`../项目文档/测试报告.md`](../项目文档/测试报告.md)。
