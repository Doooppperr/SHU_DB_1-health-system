...
本节只说明数据库对象落库后的物理组织，不重复 ER 关系和业务字段含义。当前运行库为华为云 GaussDB（openGauss 生态），数据库名为 `health_system`。项目模型中没有指定独立 schema，因此表对象按连接用户的默认 schema 创建；也没有指定独立表空间、数据文件位置、分区策略或列式存储参数，所有业务表均按 GaussDB 默认方式落为普通行存关系表。数据库存储、数据页、WAL、统计信息和底层文件管理由 GaussDB 实例统一负责，项目侧不手工管理物理数据文件。

表的主键均采用整型 `id`，由数据库自增序列生成，主键约束在物理上形成唯一索引，用于支持按主键定位和外键连接。字段存储以常规关系型类型为主：短文本使用 `String`，长文本使用 `Text`，金额和参考范围使用 `Numeric(10,2)`，日期时间使用 `Date` / `DateTime(timezone=True)`，布尔状态使用 `Boolean`，指标别名使用 `JSON`。体检报告原文件没有作为 BLOB 或大对象写入数据库，数据库只保存文件访问路径和 OCR 文本；这可以避免报告文件占用数据库数据页，文件本体由后端上传目录管理。

索引采用数据库默认的 B-tree 访问方法。主键约束、唯一约束会生成对应的唯一索引，例如用户名、邮箱、指标编码、机构分院、机构套餐、亲友关系和档案指标唯一性。模型中显式设置 `index=True` 的外键或查询字段会生成普通二级索引，主要包括 `owner_id`、`institution_id`、`record_id`、`indicator_dict_id`、`category_id`、`exam_date`、评论用户和评论机构等字段。当前未设计哈希索引、全文索引、位图索引或额外复合索引，原因是现有演示数据量较小，单列索引已能覆盖主要查询入口。

约束和触发器作为数据库端物理对象保存。外键约束由数据库维护引用关系，CHECK 约束在写入数据页前校验角色、状态、评分、价格、来源类型和非空文本等规则；三个触发器在 `health_records`、`comments`、`health_indicators` 写入或更新前执行，用于校验套餐机构一致性、评论前置档案条件以及自动维护异常标记。当前没有独立的物理迁移目录，普通表结构由 `db.create_all()` 创建，补充约束和触发器由 `backend/scripts/apply_gaussdb_rules.py` 落库维护。
