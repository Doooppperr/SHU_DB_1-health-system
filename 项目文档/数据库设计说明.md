...
当前项目采用“华为云 GaussDB 运行库 + Linux ECS 公网演示 + 本地开发备用模式”的运行方式。公网演示是当前对外展示主路径；本地前后端通过 SSH 隧道访问云库仍保留为开发和调试方式。

```mermaid
flowchart LR
    A["合作伙伴浏览器"] --> B["ECS 前端 Vite Preview 0.0.0.0:4173"]
    B --> C["ECS 后端 Waitress 127.0.0.1:5050"]
    C --> D["华为云 GaussDB 192.168.0.31:8000"]

    E["本机开发浏览器"] --> F["本机 Vue/Flask"]
    F --> G["SSH 隧道 127.0.0.1:15432"]
    G --> D
```
...
| 数据库名 | `health_system` |
| 应用账号 | `health_app` |
| 公网演示连接方式 | ECS 后端直连 GaussDB 内网地址 `192.168.0.31:8000` |
| 本地开发连接方式 | 后端连接本机 `127.0.0.1:15432`，由 SSH 隧道转发到 GaussDB 内网地址 |
...
TARGET_DATABASE_URL=opengauss+psycopg2://health_app:<HEALTH_APP_PASSWORD>@192.168.0.31:8000/health_system?client_encoding=utf8
...
| ECS 后台服务 | `scripts/linux/setup-public-demo.sh`、`health-backend.service`、`health-frontend.service` |
...
> GaussDB/openGauss 能够自动完成大部分事务和锁保护。即使本项目没有手动写 S/X 锁，数据库也会在并发读写时通过 MVCC、事务隔离和内部锁管理保证数据库层面的一致性。普通读通常使用 MVCC 一致性读；写同一行时数据库会自动加排他性质的行级锁并串行化执行。因此，不经特殊应用层锁设计，也能防止并发写入把数据库结构或约束破坏。

需要注意的是，数据库自动锁保护不等于自动解决所有业务并发语义。数据库能保证数据不会被写坏、约束不会被破坏，但不会自动判断“两个用户谁的修改应该保留”。例如两个用户同时修改同一条档案同一字段时，当前系统没有版本号校验，业务层表现可能是后提交者覆盖先提交者。若要严格防止这类丢失更新，需要额外设计乐观锁版本字段，或在关键读改写流程中使用 `SELECT ... FOR UPDATE` / SQLAlchemy `with_for_update()`。
...
本项目的物理数据库采用华为云 GaussDB（openGauss 生态）作为运行库，数据库名为 `health_system`，后端应用使用 `health_app` 账号连接。公网演示环境中，Flask/Waitress 后端部署在 Linux ECS 上，通过同一 VPC 内网地址 `192.168.0.31:8000` 访问数据库；本地开发环境不直接暴露数据库公网端口，而是通过 SSH 隧道把本机 `127.0.0.1:15432` 转发到云端 GaussDB。真实连接串、密码等敏感信息只保存在 `backend/.env` 或服务器本地环境文件中，不进入项目文档和代码仓库。

数据库对象主要由 SQLAlchemy 模型生成，启动时在 `backend/app/__init__.py` 中执行 `db.create_all()`，并由 `backend/app/seed.py` 写入基础机构、套餐、指标字典和默认管理员数据。各业务表统一采用整型 `id` 作为主键，依赖数据库自增序列生成标识；字段类型按业务含义映射为 `String`、`Text`、`Numeric(10,2)`、`Date`、`DateTime(timezone=True)`、`Boolean` 和 `JSON`。体检报告文件本身不存入数据库，数据库只保存 `report_file_url` 和 OCR 文本等元数据，上传文件由后端 `UPLOAD_DIR` 目录保存并通过 `/uploads` 路由访问。

物理访问路径以主键、外键和高频查询字段为重点。用户、机构、套餐、档案、指标和评论等表均有主键索引；常用关联字段如 `owner_id`、`institution_id`、`record_id`、`indicator_dict_id`、`exam_date` 等在模型中设置了索引，便于按用户查询档案、按机构查询评论、按体检日期和指标生成趋势。唯一约束用于保证 `users.username`、`indicator_dicts.code`、机构分院、套餐名称、亲友关系和档案指标不重复；CHECK 约束和触发器负责在数据库层兜底校验状态值、评分范围、价格非负、套餐机构匹配、评论前置档案和指标异常标记。

后端访问数据库时使用 Flask-SQLAlchemy 的连接池，默认基础连接数为 `10`，最大溢出连接数为 `20`，连接等待时间为 `30` 秒，连接回收时间为 `1800` 秒，具体值可通过 `DB_POOL_SIZE`、`DB_MAX_OVERFLOW`、`DB_POOL_TIMEOUT` 和 `DB_POOL_RECYCLE` 调整。当前项目没有单独配置表空间、分区表或独立 Alembic 迁移目录；云端补充约束和触发器由 `backend/scripts/apply_gaussdb_rules.py` 维护，历史 SQLite 数据迁移到 GaussDB 则由 `backend/scripts/migrate_sqlite_to_cloud.py` 完成。
