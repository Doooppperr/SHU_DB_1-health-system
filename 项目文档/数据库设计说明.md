...
本项目数据库物理设计重点是数据在 GaussDB 中的存储组织。当前运行库为华为云 GaussDB（openGauss 生态），数据库名为 `health_system`。项目未在模型或脚本中指定独立 schema、表空间、数据文件位置、分区方式或列式存储参数，因此所有业务表均按照 GaussDB 默认方式存放在默认存储空间中，采用普通行存关系表结构。数据库底层的数据页、事务日志、统计信息和物理文件由 GaussDB 实例统一管理，项目代码不直接维护物理数据文件。

表记录采用行式存储，一行保存一条业务记录。主键统一使用整型 `id`，由数据库序列生成，避免使用较长业务字段作为主键。字段落库类型以常规关系型类型为主，包括 `VARCHAR`、`TEXT`、`NUMERIC`、`DATE`、带时区时间戳、`BOOLEAN` 和 `JSON` 等。体检报告原文件没有作为 BLOB 或大对象写入数据库，库内只保存文件路径和 OCR 文本，文件本体由后端上传目录保存，从而减少数据库数据页和备份体积。

从存储增长角度看，机构、套餐和指标字典属于基础数据，变化较少；用户、档案、指标明细和评论属于业务数据，会随使用逐步增长。当前项目数据规模处于演示级别，因此没有按日期、用户或机构进行水平分区，也没有为冷热数据设计独立存储区域。

索引是本项目主要的辅助存储结构。主键和唯一约束会在数据库中形成 B-tree 唯一索引，常用外键和查询字段则建立普通二级索引，用于减少按用户、机构、档案和日期查询时的全表扫描。当前未额外创建哈希索引、全文索引、位图索引、聚簇索引或多列复合索引，整体物理结构保持为“普通行存表 + 序列 + B-tree 索引”的简化方案。
