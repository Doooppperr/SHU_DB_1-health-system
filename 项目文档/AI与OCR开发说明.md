...
> 文档状态：融合 1.0—3.0 的 AI、OCR 与 RAG 设计说明。当前运行版本为 3.0/schema v8，更新于 2026-07-21。

本文先说明能力如何演进，再给出当前 AI 流式交互、健康报告分析、OCR 解析和机构报告生产契约。通用角色与数据库约束分别见《项目需求与技术方案》和《数据库设计说明》；本文件出现的接口、状态和安全边界均以 3.0 当前实现为准。
...
schema v8 没有把 AI 会话写入数据库，也没有建立“AI 诊断”实体。数据库只保存真实业务产生的报告、指标、文本结果、附件和访问审计。
...
- 本人或当前仍授权的亲友健康数据才可访问。撤权或账号停用后，下一次请求立即重新鉴权。
