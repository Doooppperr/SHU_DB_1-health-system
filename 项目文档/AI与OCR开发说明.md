...
> 适用版本：当前本地 schema v3，更新于 2026-07-16。本文只描述当前接口和运行边界。
...
| 400 | `record_not_confirmed`、`record_has_no_indicators`、`mixed_record_owners` | 报告不满足分析条件 |
...
当前自动化覆盖 AI 同意与隐私边界、角色隔离、OCR Mock、草稿指标、锁定删除临时文件、机构范围隔离及发布后数据可见性。真实外部服务凭据不进入自动化测试。
