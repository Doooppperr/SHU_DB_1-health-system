...
- 文档补充：
  ...
  - 新增 `项目文档/部署演示与云数据库指南.md`（12分钟答辩演示流程）
...
- 新增 `项目文档/数据库规范化说明.md`，说明关系模式、候选键、3NF 判断与 BCNF 强化点。
...
- 状态：`Closed`
- 日期：2026-06-07
- 目标：根据“用户标识与鉴定”课堂要求，在现有用户名、密码基础上增加第三重图片验证码挑战，并覆盖登录和注册两个入口。

### 后端改动
- 新增 `GET /api/auth/captcha`，生成 4 位图片验证码，返回 `captcha_id` 和 SVG data URL。
- 登录与注册接口均要求提交 `captcha_id` 和 `captcha_answer`。
- 验证码使用内存短期挑战保存，默认有效期由 `CAPTCHA_TTL_SECONDS=300` 控制。
- 验证码校验后立即消费，不能重复使用。
- 注册成功后直接返回 access token、refresh token 和用户信息，避免注册后再走一次需要验证码的登录流程。

### 前端改动
- 登录页新增验证码输入框、图片展示与点击刷新。
- 注册页新增验证码输入框、图片展示与点击刷新。
- 登录/注册失败后自动刷新验证码，避免重复提交已消费挑战。
- Pinia 注册流程支持保存注册接口返回的登录态。

### 测试与验证
- 后端全量测试：`cd backend && .\.venv\Scripts\python.exe -m pytest` -> `33 passed`。
- 前端构建：`cd frontend && npm run build` -> `成功`。
- 手工接口验证：注册带正确验证码返回 `register success`，并包含 access token。

### 文档同步
- 更新 `README.md` 与 `backend/.env.example`：补充登录/注册验证码与 `CAPTCHA_TTL_SECONDS`。
- 更新 `项目文档/项目需求与技术方案.md`：认证模块改为“用户名 + 密码 + 一次性图片验证码”。
- 更新 `项目文档/测试报告.md`：版本提升到 `v1.6`，后端结果更新为 `33 passed`，新增验证码测试项。
- 更新 `项目文档/部署演示与云数据库指南.md`：补充现场演示和验证码排查说明。

### 结论
- 用户标识与鉴定已从两要素（用户名、密码）增强为三要素（用户名、密码、图片验证码），满足课程截图中的随机挑战/验证码要求。
...
- 时间：2026-04-08
- 变更类型：需求变更
- 详情：新增文档 `项目文档/开发记录与上下文归档.md` 用于保存历史上下文。
...
- 时间：2026-04-10
- 变更类型：决策确认
- 详情：用户确认采用“高鲁棒完整方案”，接受“上传后人工确认映射再入档”，并要求将实现结果同步到 `项目文档` 开发文档与 `开发记录与上下文归档.md`。
...
- 变更类型：文档同步
- 详情：已同步更新以下文档：
  - `项目文档/开发记录与上下文归档.md`（新增“轮次7：OCR鲁棒入档改造”）
  - `项目文档/测试报告.md`（新增 OCR 改造专项验证章节）
  - `项目文档/部署演示与云数据库指南.md`（新增“映射为0现场处理”演示策略）
  - `项目文档/开发记录与上下文归档.md`（本次完整对话与实施过程归档）
...
- 时间：2026-06-06
- 变更类型：进度更新
- 详情：数据库规范化补强完成：新增 `institutions(name, branch_name)`、`packages(institution_id, name)`、`indicator_dicts(category_id, name)` 候选键约束，并新增 `项目文档/数据库规范化说明.md`。
- 对后续影响：文档可说明数据库满足 3NF，核心字典表和关系表按 BCNF 强化。

- 时间：2026-06-06
- 变更类型：文档同步
- 详情：已同步更新 `README.md`、`项目文档/部署演示与云数据库指南.md`、`项目文档/数据库规范化说明.md`、`项目文档/部署演示与云数据库指南.md`、`项目文档/测试报告.md`、`项目文档/开发记录与上下文归档.md`。
...
- 时间：2026-06-07
- 变更类型：验证结论
- 详情：已通过事务内回滚方式验证数据库规则：评论评分越界被 `CHECK` 拦截、档案套餐机构不匹配被触发器拦截、指标异常标记由触发器自动维护。当前后端全量回归测试结果为 `33 passed`。
- 对后续影响：云库结构、代码模型、规则脚本和测试文档已对齐。

- 时间：2026-06-07
- 变更类型：需求变更
- 详情：用户根据课堂“用户标识与鉴定”截图要求，希望在现有用户名和密码之外增加第三重身份挑战；实现方式确定为登录/注册页面输入图片验证码。
- 对后续影响：认证入口需先获取 `/api/auth/captcha`，再提交 `captcha_id` 和 `captcha_answer`；验证码为一次性挑战，失败后应刷新。

- 时间：2026-06-07
- 变更类型：进度更新
- 详情：已完成认证验证码增强：后端新增验证码接口，登录和注册均校验验证码；前端登录页、注册页均新增验证码输入和刷新；注册成功直接返回并保存 JWT 登录态。
- 对后续影响：测试、演示和接口调用都需要带验证码字段；生产环境不会返回验证码答案，测试环境为自动化测试保留 `captcha_answer`。

- 时间：2026-06-07
- 变更类型：验证结论
- 详情：后端全量测试 `cd backend && .\.venv\Scripts\python.exe -m pytest` 结果为 `33 passed`；前端 `cd frontend && npm run build` 成功；手工接口验证注册成功返回 token。
- 对后续影响：认证验证码增强已达到收口条件，可用于课程演示。

- 时间：2026-06-07
- 变更类型：文档同步
- 详情：已同步更新 `README.md`、`backend/.env.example`、`项目文档/项目需求与技术方案.md`、`项目文档/测试报告.md`、`项目文档/部署演示与云数据库指南.md`、`项目文档/开发记录与上下文归档.md`。
- 对后续影响：项目说明、技术方案、测试报告、演示指南和开发归档均与验证码功能保持一致。

- 时间：2026-06-08
- 变更类型：部署能力
- 详情：完成 Linux ECS 公网演示部署：前端通过 `http://190.92.227.58:4173` 对外提供访问，后端 `health-backend.service` 仅监听服务器本机 `127.0.0.1:5050`，前端 `health-frontend.service` 代理 `/api` 与 `/uploads` 到后端；两个 systemd 服务均已设置开机自启。
- 对后续影响：合作伙伴不再需要 clone 项目、建立 SSH 隧道或本地启动服务；ECS 与 GaussDB 均运行时，服务器启动后等待服务自启即可访问公网演示地址。

- 时间：2026-06-08
- 变更类型：运维流程
- 详情：明确日常同步流程：本地修改完成后执行 `git add`、`git commit`、`git push origin main`；随后登录服务器 `/opt/health-system` 执行 `sudo bash scripts/linux/setup-public-demo.sh`，脚本会拉取最新代码、安装依赖、构建前端并重启 systemd 服务，且默认保留服务器本地 `backend/.env`。
- 对后续影响：服务器不会自动读取本地未提交代码；公网演示环境只会展示上一次成功部署的 GitHub `main` 版本。

- 时间：2026-06-08
- 变更类型：文档整理
- 详情：统一更新 `README.md`、`backend/README.md`、`项目文档/项目需求与技术方案.md`、`项目文档/数据库设计说明.md`、`项目文档/测试报告.md`、`项目文档/部署演示与云数据库指南.md`，将“当前演示方式”明确为 Linux ECS 公网演示，并将 SSH 隧道限定为本地开发/调试模式。
- 对后续影响：文档中的部署、数据库连接、测试验收和日常同步流程与当前服务器状态一致，减少后续演示和维护时走错旧流程的风险。
